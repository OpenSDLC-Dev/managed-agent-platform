- **AGENTS.md stops forbidding the release ritual's own last step, and the fragment sweep
  ...
  docs/changelog/, so a future non-archive file there is still swept; and step 9 returns
  Active work to **None** only when nothing else is in flight. Step 8 now also says what
  becomes of a fragment that lands between the release PR and the tag — it ships in that
  release but folds into the next one's section, so the fragment names its version. (#426)

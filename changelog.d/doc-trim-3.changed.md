- **ARCHITECTURE.md's package reference is a map now, not a second copy of the code** (#413) — the
  ...
  package but `internal/version` carries more comment than the document spent on it, most by three
  to ten times (`internal/api` 192 KB against 50 KB, `internal/executor` 169 KB against 43 KB).

...
(Plus test-support: `internal/{pgtest,dockertest,modeltest}`, `internal/sandbox/sandboxtest`, `internal/blob/blobtest`, `internal/blob/gcs/gcstest`, `internal/provider/providertest`, `internal/secrets/secretstest`, `internal/secrets/gcpkms/gcpkmstest`, `internal/webtool/webtooltest`, `internal/identity/identitytest`, and the top-level `evals/` live suite and `acceptance/` doc-example suite. There is no `internal/policy`: permission policy lives across `domain`/`toolset`/`brain`/`api`.) What each package's files actually do: [docs/ARCHITECTURE.md](./docs/ARCHITECTURE.md) → "Package reference".

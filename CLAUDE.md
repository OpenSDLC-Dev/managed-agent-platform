...
- Mirror Anthropic's resource model, paths, JSON fields, and **ID prefixes**: `agent_` `env_` `sesn_` (accept a `session_` alias on input too — a lenient parse of ours, unobserved on the reference) `sevt_` `work_` `vlt_` `vcrd_` `sesrsc_` `depl_` `drun_` `file_` `skill_` `skillver_` `outc_` `sthr_` — `knownPrefixes` in `internal/domain/id.go` is the list, and a test fails this line if it drifts from it.

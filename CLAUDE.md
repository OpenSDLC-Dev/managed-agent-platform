...
CI (`.github/workflows/ci.yml`) invokes the same targets, so the gate cannot drift between the docs, the verifier, and the merge check. A second workflow, `registry.yml`, runs `make registry-check` nightly and on any PR touching the registry, the guard or the Makefile — the state rung a credential-free gate cannot hold. A third, `sdk-bump.yml`, runs `make sdk-bump-report` on any PR touching `go.mod`, `tools/sdkref` or the Makefile, and fails while a transition it reports awaits a disposition: the pull request that moves a pin is where plan 51 reads a bump, and the gate deliberately is not. The coverage gate is **total statement coverage ≥ 90%** over the **logic packages** under `./internal/...` — `cmd/` main glue and test support are deliberately outside the denominator, and the `test` recipe both computes that denominator and argues the exclusion. `make test` needs a reachable Docker daemon and the `docker` CLI on PATH, which the store, API and sandbox suites drive it through, and a Kubernetes cluster (the K8s sandbox contract test; a local kind cluster works), and it runs from a git clone, not a source archive (`tools/sdkref` asks git which files are this repository's); a missing daemon, binary, cluster or clone is a hard failure, not a skip.

...
`Generate`/`Persist` minter over its pool; the K8s sidecar is slice 4d).
...
`cmd/gate`, below) and the Docker sandbox provider wires the sidecar (slice 4-i); the K8s sidecar is
slice 4d.
...
the sidecar (shared network namespace, proxy env, teardown; slice 4-i); the K8s wiring is slice 4d.

...
session's turn is one trace across process boundaries. All three deployment points of the
pull protocol wrap a claimed work item in a consumer span standing for its handling end to
end — the brain's `model_turn`, the executor's and the BYOC worker's `tool_exec` — and each
reports its own faults from inside that span, so a fault log is always reachable from the
red span it describes. Anthropic

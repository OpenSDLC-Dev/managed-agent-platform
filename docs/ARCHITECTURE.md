...
| `memsync/` | What the executor and the BYOC worker share about a mounted memory store, so the two halves of plan 36 cannot drift: the path and content rules every memory write obeys and the mount-path slug (slice 2); the marker, baseline and tree-hash conventions and the pure `Plan(local, baseline, remote)` decision table arrive with the sync (slice 4). |

...
| `k8s/client.go` | client-go construction and the pod-exec transport; documents the image contract (`/bin/bash`, `setsid`, `tee`/`wc` for the write path's delivered-byte count, `mv`/`rm` for the temp-and-rename write both backends now do, and a `stat` accepting `-c`). |

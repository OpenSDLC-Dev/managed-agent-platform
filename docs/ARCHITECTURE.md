...
| File | Contents |
|---|---|
| `id.go` | `ID` with wire-compatible prefixes (`agent_`, `env_`, `sesn_`, `sevt_`, `work_`, `vlt_`, `sesrsc_`, `depl_`, `drun_`, `file_`, `skill_`); accepts the alternate `session_` form on input. CSPRNG + Crockford base32 generator (`idAlphabet`, shared with `Valid`). `Valid()` shape-checks an id (a known prefix + a Crockford token) so the API rejects a malformed or unstorable path/query id (404/400) before it binds as a 500. |

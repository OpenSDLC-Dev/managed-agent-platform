...
| `k8s/client.go` | client-go construction and the pod-exec transport; documents the image contract (`/bin/bash`, `setsid`, GNU `stat`). |
| `k8s/deadline.go` | The in-pod exec wrapper and pid/exit-file scripts adapting the Docker deadline discipline to Kubernetes' stream-coupled exec. |
| `k8s/probe.go` | The two-instant deadline probes (alive-at-deadline / overran-plus-slop) answered by a second exec reading the pid file; sticky overrun verdict. |

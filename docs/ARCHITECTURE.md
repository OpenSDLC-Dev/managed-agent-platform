...
| Binary | Role |
|---|---|
| `controlplane` | The wire-compatible REST surface: resource CRUD, the event log endpoints (POST/list/SSE), the work API for BYOC workers, auth (management `x-api-key`, worker environment keys), and the session state machine. It also runs the five background sweeps no request drives — memory-version retention (#476), at startup and then hourly; expired-file retention (#655), at startup and then hourly, which removes a file's row once the published 30-day grace window past `expires_at` has elapsed — unless an open dream owns it, the manual delete route's own refusal — and enqueues its object in the same transaction rather than deleting it; the deployment scheduler (plan 37), at startup and then every 30 seconds; the object-delete drain (plan 50), at startup and then every minute or on the in-process wake a remover raises after its own commit, the one remover of what a transaction wrote down as owed — keys a commit recorded as it removed the rows that referred to them, which is now every path that deletes a row naming an object: session delete, the file sweep, the file, skill, skill-version and dream deletes, and the executor's harvest replacing a snapshot (#703). Every one of those but the harvest wakes it; the harvest runs in a binary that hosts no drain and so waits out the interval, which costs nothing on a path with no client behind it. What a path still deletes for itself is the object whose row never committed — owed to nothing, and actively wrong to queue — and the session checkpoint the reaper removes a second time, whose key the session delete has already enqueued; and the dream runner (plan 41), at startup and then every `DREAM_TICK_INTERVAL` — all cancelled and drained before the pool closes. The scheduler's at-most-one-run-per-occurrence guarantee across replicas is the partial unique index on `(deployment_id, scheduled_at)` — the reference's own published idempotency key — not leader election, which this platform has nowhere; and its clock is Postgres's, one `SELECT now()` per tick, so a replica with a skewed clock cannot shift what fires. The runner needs no such index and keeps no lease renewal and no owner column: a tick takes one dream's row `FOR UPDATE SKIP LOCKED`, advances it by one step and commits, so one replica works a dream at a time and a replica that dies mid-tick costs the step it was in. The one claim that does span ticks is the start's — a constant against `updated_at` that keeps a claimed dream out of every replica's scan, and that a crashed claimant burns rather than renews. |
...
| Package | What it owns |
|---|---|
| `egress/` | The substitution engine that rewrites vault placeholders into secrets, the host matcher both it and the gate use, and the one host list belonging to neither an operator nor a credential — the curated package registries `allow_package_managers` opens, which lives here because the gate and the control plane both have to be told it. Beside them, the MCP admission predicate (`MCPServerAdmitted`), which session create and the executor's dial both ask. It holds no I/O, and that is the invariant to keep. |
...
- **Auth is scoped.** Management calls carry `x-api-key` (hashed at rest,
  ...
  lease or an archive ends it; a stop leaves it working while the item is `stopping`
  and, once it is `stopped`, for a minute from the request (the reference worker
  learns of the stop at its next heartbeat, then flushes its memory writes for up to
  30 s on a context of its own), and the queue settles a wind-down whose worker went
  silent only once that same minute (`queue.WindDown`) is over, so no settlement
  re-arms a session while its token works.

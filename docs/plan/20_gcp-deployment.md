...
- **Warm-pool sandbox provisioning** (kubernetes-sigs/agent-sandbox `SandboxWarmPool` /
  `SandboxClaim`): cold-start optimization, its own plan. Two integration hazards belong
  in that plan's problem statement rather than being rediscovered there. The gate sidecar
...
3. **Cipher: `internal/secrets/gcpkms`, KMS `Encrypt`/`Decrypt` directly — this is what
   ...
   **Envelope encryption** — a local AES-256-GCM data key wrapped by KMS, which removes
   the ceiling entirely and cuts KMS round-trips — is the recorded follow-up, deferred
...
9. **A persistent staging environment with one-command create and destroy.** The cluster
   ...
   **Terraform** under `deploy/gcp/`, wrapped by Make targets — the same shape Google's
   ...
   So `deploy/gcp/` is **two Terraform configurations, not one**, and the split is along
   exactly this line:
   - **`foundation/` — created once, never destroyed.** The KMS key ring and crypto key,
     and the Secret Manager secrets. `terraform destroy` is not a supported operation
     here; the resources carry `prevent_destroy`, and the Make target that tears the
     environment down does not touch this state. It costs nothing idle.
   - **`environment/` — created and destroyed freely.** Cluster, node pools, Artifact
     Registry, service accounts, IAM bindings, Cloud SQL, the GCS bucket. It reads the
     foundation's key and secret names through `data` sources, never owning them.
   ...
   Acceptance is therefore **create → destroy → create** on `environment/`, with
   `foundation/` untouched throughout, and the second create must come up against the
   *same* KMS key — proven the only way that means anything: a vault credential written
   before the destroy is read back successfully after the rebuild.
10. **Sandbox pods get a disk limit *and* a node of their own.** The exposure is real
    ...
    eviction is **targeted, attributable, and bounded to the pod that caused it**, and
    the node stays healthy. Targeted beats arbitrary on every axis that matters here.
    ...
    Two mechanisms exist, not one, and they cover different bytes: a container
    `ephemeral-storage` limit bounds the writable layer, which is where the workspace
    lives; `emptyDir.sizeLimit` bounds the emptyDir volumes, which exist only when
    `SANDBOX_READ_ONLY_ROOTFS` is on. Slice 2 adds the former as `Hardening`
    configuration with an empty default — today's behaviour stays the default, and the
    GCP deploy guide sets a real value. The latter is noted for the read-only-rootfs
    configuration and left to the operator.
    ...
    **Node isolation is the second half, and it needs code.** The sandbox pod spec
    ...
    sandbox from taking down its co-tenants, which are other sandboxes; the **dedicated
    pool** stops it from reaching the control plane, brain, executor and collector at
    all. Node isolation alone would only relocate the tenancy problem, which is why this
    decision does both.
11. **The GCS deployment identity needs bucket-read on top of object-admin.** Measured
    ...
    S3-compatible backend rather than just GCS, and is recorded as a follow-up issue
...
- **Slice 2 — sandbox pod placement and bounds (`internal/sandbox` +
  ...
  - **An `ephemeral-storage` limit**, as a `Hardening` field with an empty default
    (Decision 10). Kubernetes-only, like `PidsLimit` is Docker-only: Docker's writable-
    layer quota needs a specific storage driver, so the asymmetry is registered in
    docs/DIVERGENCES.md the way #65 registered its own rather than faked.
  - **Node selection and tolerations** on the sandbox pod, as provider `Config` fed by
    ...
    precedent exactly, and for the same reason: which nodes a cluster has is a property
    of the cluster, not of a session. Without this, Decision 10's tainted sandbox pool
    would leave every sandbox `Pending`, so it is a prerequisite for slice 4's
    environment rather than an enhancement.

  Chart surface for all three, defaults preserving today's behaviour. Unit-test at the
  podSpec level, and run the shared contract suite on kind **including a gated session**:
  a seccomp filter that interfered with the gate's `iptables-restore` under `NET_ADMIN`
  would surface there and nowhere else, and this slice must prove it does not.
...
- **Slice 5 — mode-2 acceptance + `docs/deploy-gcp.md`.** Cloud SQL (private IP,
  ...
  database** — DDL-capable within that database because all three binaries run
  migrations through `store.Open` at startup, and nothing wider, since the migrations
  are ordinary schema DDL with no role or database creation), GCS (pre-created bucket,
  ...
  actually tested. Write the deploy guide: the Gateway + managed-certificate example and
...
project, `terraform destroy` tears it down, and a second `apply` rebuilds it. Both
acceptance runs recorded in docs/HISTORY.md.
...
the set. Scoped: the GCS HMAC pair (single bucket; its removal is the
GCS-native-backend follow-on), the model key (the provider's own), and the **Cloud SQL
user, which owns the platform's own database and nothing else** — DDL-capable within
that database, because `store.Open` migrates the database its DSN names and the
migrations are ordinary schema DDL with no role or database creation, so this is
deployment-level least privilege rather than an escape hatch. Unscoped: the
**management API key, which authorizes every management `/v1` route with no role or

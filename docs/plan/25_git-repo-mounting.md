...
| w-mount-bounds | 🔍 | `mount_path` an absolute path longer than `maxMountPathBytes` (1024); an absolute path carrying a U+0000 | 400 each. The length rule is `validateRepoMountPath`'s own, but the NUL is refused a layer earlier by the body-wide `rejectNUL` decode walk, so what this row pins is the **outcome** — a 400, never the #135 500 at the jsonb bind — not which guard fires. No invalid-UTF-8 case exists because JSON cannot carry one: the marshaller substitutes U+FFFD before the request leaves the client, so `storableText`'s UTF-8 branch is unreachable from the wire and stands as defence in depth (established by mutation probe, 2026-08-07: disabling `storableText` in `validateRepoMountPath` leaves Unit W green) |
...
| w-geometry-ok | 🔍 | overlay: a repo at `/mnt/session/uploads/r` + a file supplied as `r/src/x`; disjoint: a file supplied as `/workspace/repo` + a repo at `/workspace/repo/src` | 200 for both pairs (this server's create status everywhere, per `w-create-minimal`) — the overlay is decision 5's supported direction, and the disjoint pair only *looks* nested (the file is stored at `/mnt/session/uploads/workspace/repo`). A 400 on either is a **FAIL**: it would mean a rule compared supplied spellings instead of stored paths |

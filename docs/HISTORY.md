...
The full compose stack (controlplane + brain + executor + Postgres + MinIO + OpenBao + the built gate image), the brain routed to a real Anthropic-protocol gateway (a MiniMax endpoint from `.env`), the executor opted into the gate (`EXECUTOR_GATE_IMAGE` + `CONTROLPLANE_URL`). All resources created by the **real `ant` CLI** built from the local checkout, driving the control plane over `--base-url`: a vault (`vlt_…`), an `environment_variable` credential (`secret_name: HTTPBIN_TOKEN`, `networking.allowed_hosts: [httpbingo.org]`, header-only injection, a sealed `secret_value`), a `limited` environment (`allowed_hosts: [httpbingo.org]`), a bash-toolset agent, and a session with the vault attached. **The plan names `httpbin.org`; its public instance was returning 503 during the run, so the reachable plain-HTTP header-echo host `httpbingo.org` was substituted — the only deviation.** The default `debian:stable-slim` sandbox image carries no HTTP client, so a one-off `debian:stable-slim + curl` image was set as `EXECUTOR_IMAGE` for the run (the sandbox base is a documented deployment choice).
...
- **Revocation half 2 — a reused pre-archive placeholder is no longer substituted.** Replaying `vltph_8bc51b78…` in an `Authorization` header after archival, httpbingo echoed **the placeholder literally** (`"Bearer vltph_8bc51b78b22e6faff3046b2ae87342c4"`), not the secret: the engine resolves at egress from current rows, and the archived credential's ciphertext cannot resolve. The host stayed reachable (the environment's `allowed_hosts` is independent of the credential) — only the secret is gone.

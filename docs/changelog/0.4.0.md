...
- **Memory versions no longer accumulate forever** (#476) — the control plane now
  ...
  [docs/DIVERGENCES.md](../DIVERGENCES.md) carries the count and what it rests on.
...
- **Memory stores are now covered where plan 36 said they would be and were not**
  ...
  [docs/self-hosted-security.md](../self-hosted-security.md) §2 and §4 already
...
- **Plan 37 archived — scheduled deployments** ([#51](https://github.com/OpenSDLC-Dev/managed-agent-platform/issues/51), [docs/plan/37_scheduled-deployments.md](../plan/37_scheduled-deployments.md)) — six slices, from the cron engine and the deployment schema to the run lists: a deployment binds an agent to an environment, credentials, resources and initial events; `POST /run` fires it by hand, a 5-field POSIX cron schedule fires it on the clock — at most one run per occurrence across every replica, bounded catch-up, auto-pause on the fourteen pausing error types — and `/v1/deployment_runs` reads the history back. The close-out adds the two security-invariant bullets to [docs/ARCHITECTURE.md](../ARCHITECTURE.md): a scheduled session's `created_by` is NULL by design (a schedule is nobody), and a schedule does not bypass the permission policy — an `always_ask` tool call parks the session on a pending confirmation, unreapable until a human answers, interrupts or archives it, deliberately not refused at create. What the plan excluded is named and filed: webhooks ([#261](https://github.com/OpenSDLC-Dev/managed-agent-platform/issues/261)), budgets ([#432](https://github.com/OpenSDLC-Dev/managed-agent-platform/issues/432)), jitter and the overlap brake ([docs/DIVERGENCES.md](../DIVERGENCES.md)). The progress summary is in [docs/HISTORY.md](../HISTORY.md).
...
- **Six registry entries stopped naming a closed issue as their live tracker** (#502) —
  ...
  [docs/DIVERGENCES.md](../DIVERGENCES.md) entries still cited it as *live*, so each

...
own pointer line below — and its per-file reference in the packages' own doc comments. (Until
...
reference: the packages' own doc comments — plan 34 slice 3 deleted ARCHITECTURE's
per-file tables, which said less than the code they described.

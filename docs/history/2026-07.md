...
Sections of [docs/HISTORY.md](../HISTORY.md) whose events belong to
2026-07, moved by plan 28 slice 2 — unchanged at the time, apart from
relative links re-based for this directory. An archive is an ordinary
document, not a frozen one (`docs/changelog/` is the frozen kind): plan
34 has since edited it, so the two files no longer recompose to the
pre-split file byte-for-byte. A newer month stays in HISTORY.md until it
archives.
...
own pointer line below — and its per-file reference in the packages themselves: the doc
comments, or the files where the substance is unexported (`api`, `brain`, `executor`). (Until
plan 34 slice 3 that reference was ARCHITECTURE.md § "Package reference"; those tables are
gone, having said less than the code they described.) The headings survive as citation
anchors for [DIVERGENCES.md](../DIVERGENCES.md); where a citation's parenthetical quotes
prose from a pruned body, that prose lives on in the matching CHANGELOG entry — or, for the
one case whose only home was a deleted ARCHITECTURE row, in the citing DIVERGENCES entry
itself, which restates the fact in full. What remains under a heading is recorded nowhere else.
...
- **No `Attach`.** The plan's `SandboxProvider` had `Provision` + `Attach(sandboxID)`. `Provision` is instead idempotent per session — it adopts the session's running container — which is the only thing an executor ever needed `Attach` for, and it spares us persisting a sandbox id nothing else would read. — **Since partly reversed** (2026-08-14, plan 29 slice 4c, `e0b5cad`): `SandboxProvider` has an `Attach` again, for a caller this decision did not have — one that wants the sandbox a session *has* rather than the one it is about to use. It keys on the session id, not a sandbox id, so the reason recorded here survives its own reversal.
...
reference: the packages themselves — their doc comments, or the files where the substance
is unexported. Plan 34 slice 3 deleted ARCHITECTURE's per-file tables, which said less
than the code they described.

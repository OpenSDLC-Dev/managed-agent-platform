...
- Scheduled deployments. Memory stores are landing under [plan 36](./docs/plan/36_memory-stores.md): the `/v1/memory_stores` surface serves, and a `cloud` session's stores are mounted at `/mnt/memory/<slug>`, told to the agent, and synced back when each tool run ends; a `self_hosted` session cannot attach one yet.

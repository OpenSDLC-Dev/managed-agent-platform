...
- **Vaults and egress** — `/v1/vaults` holds cipher-sealed, write-only credentials that a session attaches at create time; a `limited` environment's traffic leaves through a per-session egress gate that enforces `allowed_hosts` — widened only by the two flags beside it, which open the agent's declared MCP endpoints and a curated thirty-host set that reaches package registries, source forges and container registries alike — and substitutes those credentials on the way out, so the sandbox only ever holds an opaque placeholder.

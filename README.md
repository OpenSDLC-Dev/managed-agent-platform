...
| Live-system evals | `RUN_EVALS=1` (`make eval`) | whole sessions: API → brain → real model → sandbox → SSE, deterministically graded. [Eleven regression tasks](./docs/plan/02_evals-system.md) spanning the built-in toolset, permission allow/deny, single- and multi-turn, and skill injection; results land in `evals/artifacts/`. CI also runs this tier daily on a schedule, from repo `MODEL_*` secrets ([`evals.yml`](./.github/workflows/evals.yml)) |

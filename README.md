...
Deferred past v1 (seams reserved, not implemented — each tracked as an issue): secret vaults and egress credential injection, scheduled deployments, memory stores, multi-agent threads, and multi-tenant RBAC/SSO. Skills left that list and are in active development ([#54](https://github.com/OpenSDLC-Dev/managed-agent-platform/issues/54)): the `/v1/skills` registry over object storage, the anthropic prebuilt-skills import, and sandbox materialization on both execution halves have landed; prompt injection is in progress.

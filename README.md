...
- **Scheduled deployments** are landing under plan 37 ([#51](https://github.com/OpenSDLC-Dev/managed-agent-platform/issues/51)): the `/v1/deployments` surface serves and a cron schedule is validated, stored and echoed with its upcoming occurrences — but nothing fires one yet, neither a manual run nor a scheduler.

...
- **Dreams** — a dream consolidates one memory store over 1–100 past session transcripts. The control plane renders each transcript with secrets redacted and its size capped, mounts them beside a clone of the store, and runs a pipeline session under the model the request names — so what the job rewrites is the clone, which you review and keep or discard, and the store you fed it is untouched. The five `/v1/dreams` routes are the reference's, `ant beta:dreams` drives them, and the pipeline behind them runs four stages — orient over the transcripts, digest them through one delegated thread per batch, merge the digests into the store, then rewrite its index and report. Asking for `update_existing` instead consolidates the store you named in place, at most one such job per store at a time; that session runs with no shell, so it can create and change memories but never delete one, and a memory it retires is left as a tombstone naming its successor for you to remove.
- **Outcomes** — a text or file rubric is graded each work cycle, and revision feedback runs up to `max_iterations`.
- **Session deliverables** — whatever the agent writes under `/mnt/session/outputs/` in a **cloud** session is harvested into the Files API, with or without an outcome, and downloads through `GET /v1/files?scope_id=<session>`. Any of five brain settlement folds triggers a harvest — a plain `end_turn`, a retries-exhausted failure, a confirmation gate, a delegated park, and a spent delegation budget — not literally every idle: the two folds that *end* an outcome cycle already harvested the same tree, and the two idle folds the API makes (a `user.interrupt`, a session's last-thread archive) are tracked separately by [#586](https://github.com/OpenSDLC-Dev/managed-agent-platform/issues/586). A `self_hosted` session never harvests, because the platform cannot reach a BYOC sandbox. Deleting the session takes what it harvested with it, rows and bytes both; archiving keeps them, and a file you uploaded through the Files API is untouched by either.
- **Vaults and egress** — `/v1/vaults` holds cipher-sealed, write-only credentials that a session attaches at create time; a `limited` environment's traffic leaves through a per-session egress gate that enforces `allowed_hosts` — widened only by the two flags beside it, which open the agent's declared MCP endpoints and a curated thirty-host set of package-registry, source-forge and container-registry hosts (host reach is what the recording establishes; whole clones and image pulls are not) — and substitutes those credentials on the way out, so the sandbox only ever holds an opaque placeholder.
...
- **Scheduled deployments** — a deployment binds an agent to an environment, credentials, resources and initial events; `POST /run` fires it by hand, and an optional 5-field POSIX cron schedule fires it on the clock — at most one run per occurrence across every control-plane replica, a bounded catch-up after downtime, and a classified fire failure recording an error-bearing run that auto-pauses the schedule (an unclassified one rolls back whole and retries — [docs/DIVERGENCES.md](./docs/DIVERGENCES.md) registers the trade). `/v1/deployment_runs` reads the history back.
...
Deferred past v1 — **seams reserved, not implemented**, each tracked as an issue. The canonical list is the open issues whose titles carry the `(post-v1)` marker (`gh issue list --state open --limit 100 --search "post-v1 in:title"`, which also returns the occasional issue that merely mentions the phrase); the notes below single out what most often surprises a reader, and are neither the whole of that set nor confined to it. Vaults, skills, files, repository mounting, memory stores and scheduled deployments were on this list and have since landed; what remains includes:

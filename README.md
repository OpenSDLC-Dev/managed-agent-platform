...
| Live-system evals | `RUN_EVALS=1` (`make eval`) | whole sessions: API → brain → real model → sandbox → SSE, deterministically graded. [Seventeen regression tasks](./docs/plan/02_evals-system.md) spanning the built-in toolset, permission allow/deny, single- and multi-turn, skill injection, file, repository and MCP-server mounting, outcome grading, and coordinator delegation across two worker threads; results land in `evals/artifacts/`. CI also runs this tier daily on a schedule ([`evals.yml`](./.github/workflows/evals.yml)), reading the same four model variables plus `EVAL_GITHUB_REPO_URL` / `EVAL_GITHUB_REPO_TOKEN` from the `evals` deployment environment's secrets — and failing rather than skipping while they are unset |

...
- **Scheduled deployments** are landing under plan 37 ([#51](https://github.com/OpenSDLC-Dev/managed-agent-platform/issues/51)): the `/v1/deployments` surface serves, `POST /run` fires a manual run, the scheduler fires the schedule itself, and `/v1/deployment_runs` reads the history back — what remains is the close-out documentation.

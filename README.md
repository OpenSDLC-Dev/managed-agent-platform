...
- **Scheduled deployments** are landing under plan 37 ([#51](https://github.com/OpenSDLC-Dev/managed-agent-platform/issues/51)): the `/v1/deployments` surface serves, a cron schedule is validated, stored and echoed with its upcoming occurrences, and `POST /run` fires a manual run — but no scheduler ticks yet, so the schedule itself starts nothing.

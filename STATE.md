...
- [x] **Slice 3 — executor materialization + streaming sandbox write + brain injection + `file-answer` eval** (`Sandbox.WriteFileStream` on both backends; `materializeFiles` streams blob→sandbox with sentinel + `test -e` re-probe; brain "Mounted files" block; evals upload→mount→read). — PR pending.

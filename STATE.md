...
- [ ] Slice 4 — activation. **4a done**: the execution driver and the MCP-first settlement,
      shipped ahead of the emission so a rollout cannot strand a call ([#398](https://github.com/OpenSDLC-Dev/managed-agent-platform/pull/398)).
      **4b done**: the brain offers an `mcp_toolset`'s tools as `mcp__{server}__{tool}`,
      commits `agent.mcp_tool_use`, and waits for a first listing. **4c left**: output spill.

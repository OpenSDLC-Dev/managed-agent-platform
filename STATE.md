...
| `brain/brain.go` | Claim → replay → generate → emit → settle. Reclaimed items surface `session.status_rescheduled` + `status_running` before rerunning. Settlement: `tool_use` suspends (session stays running; the result event's enqueue resumes it); `end_turn` idles with `end_turn` unless input arrived mid-turn, in which case the turn **requeues its own item** (a fresh enqueue would be suppressed by its live slot); provider/config failures append `session.error` (`model_request_failed_error`, `retry_status: exhausted`) + idle `retries_exhausted`. Usage fold + processed watermark commit with settlement. |
...
1. **Slice 6:** tool-exec executor + Docker sandbox provider + built-in `agent_toolset` expansion (the tool-exec queue kind and its enqueue seam already exist), consuming the `agent.tool_use` intents the brain emits.

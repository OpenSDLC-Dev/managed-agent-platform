...
- [ ] Slice 4 — egress gate phase 1, landing as sub-PRs. **Done:** `sandbox.Spec.Env` seam threaded into the Docker (`Env` in container config) and K8s (container `Env`, `$(VAR)` expansion neutralized) backends with `ValidateEnv` key-grammar gating, contract rows `SpecEnvReachesExec`/`SpecEnvRejectsInvalidKey`/`SpecEnvBoundAtProvision` (PR #172). **Remaining:** read-time resolution + substitution-engine packages, per-session gate proxy + `HTTP(S)_PROXY` injection + `vltph_` placeholder minting, K8s UID-owner iptables sidecar (replacing route-flush), contract rows "limited = only allowed_hosts" + DIVERGENCES.md:42 superseded.

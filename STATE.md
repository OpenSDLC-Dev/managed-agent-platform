...
**Slice-3 wire/scope decisions (documented divergences):** the stream is a live tail from connect time — no history replay, no `Last-Event-ID` (the reference client parses neither; reconnecting clients seed via list). `tool_use_id`/`custom_tool_use_id` existence is not cross-checked on send. `user.define_outcome` is rejected (outcome surface deferred; no `outc_` prefix in v1). A non-null `session_thread_id` is rejected (threads deferred). `seq` is internal only — never a wire field; cursors carry it opaquely.
...
| `api` state machine | `POST /events` is one transaction (`FOR UPDATE OF s`): `user.message` on an idle session → running + `session.status_running` + model_turn enqueue; a tool result while running → next model_turn, no status event (awaiting a tool is still `running`); everything else appends only. The response echoes only the posted events. Session updates emit `session.updated` with only the changed fields (title / non-empty metadata / agent snapshot). |
| `brain/brain.go` | Claim → replay → generate → emit → settle. Reclaimed items surface `session.status_rescheduled` + `status_running` before rerunning. Settlement decides the work item's fate **in its own transaction** (the liveness guarantee: API triggers and settlement serialize on the session row lock, so a tool result posted mid-settle either sees the live item — suppressed enqueue, but the settle check catches the landed result and requeues — or a completed one, never a gap): `tool_use` suspends (session stays running; a result that already landed chains immediately); `end_turn` idles with `end_turn` unless input arrived mid-turn, in which case the turn **requeues its own item** (a fresh enqueue would be suppressed by its live slot); provider/config failures append `session.error` (`model_request_failed_error`, `retry_status: exhausted`) + idle `retries_exhausted`. Usage fold + processed watermark + item completion commit together. The staleness check runs before reclaim recovery, so a reclaim of already-finished work never flips an idle session back to running. |
...
| `brain/stream.go` | Provider chunks → wire: `agent.message` preview opened at first text delta (provider block index → content entry index), `agent.thinking` preview per block (start-only, buffered event under the preview id), tool_use collected, the buffered `agent.message` lands **before** `span.model_request_end` (the SDK accumulator closes previews at span end), lease re-extended mid-stream. |
...
**Slice-5 decisions (documented assumptions, revisit against reference behavior when observable):** the tool_use id the model sees on replay is the `sevt_` event id; a session awaiting a tool result stays `running` (`requires_action` is reserved for slice-7 confirmations); `max_tokens`/`stop_sequence` settle as `end_turn`; tool-only turns emit no empty `agent.message`; the flat cache-creation counter folds into the `ephemeral_5m` bucket; no automatic retry — one failed model request ends the turn visibly; `user.interrupt` is logged but not yet acted on; `session.status_running` is emitted once per idle→running flip, not per tool result; v1 never requests extended thinking, so continuations legally omit thinking blocks (the slice-4 signature note stands for when thinking becomes configurable). `agents`/`sessions` wire structs now live in `internal/domain` (`AgentSpec`/`ResolvedAgent`/`Usage` are the wire shapes; the slice-2 debt is settled). `internal/pgtest` is the shared Docker-Postgres harness for new packages; migrating the three older private copies stays a chore.

**Test coverage:** queue contract tests (idempotent enqueue, kind isolation, FIFO, parallel claims share nothing, expired-lease reclaim + lost-lease `Complete`/`Extend`/`Requeue` failures); `AppendWith` atomicity (Then-error rollback, usage accumulation, watermark stamping); API state-machine tests (flip + enqueue exactly once, tool-result resume, no-op update emits nothing, `session.updated` carries only changed fields); brain turns against a scripted provider over real Postgres (full-turn event order, tool suspend/resume with event-id linkage, mid-turn message chaining via requeue, provider error → `session.error` + `retries_exhausted`, unrouted model fails visibly, reclaim surfaces recovery, `Run` drains and stops); white-box replay tests (role merging, result-first ordering, malformed events rejected).

...
| `sandbox/docker/api.go` | ~9 Engine API endpoints over `net/http` on the daemon socket (`DOCKER_HOST`, `unix://` or `tcp://`), plus the exec stream's frame demultiplexer. Hand-rolled: the official client would pull the whole moby module tree in for this. |
| `sandbox/docker/docker.go` | `Provision` (inspect → create → pull-on-missing-image → adopt-on-409 → start), `Exec` under an in-container watchdog, tar-based file transfer with parent creation on the archive endpoint's 404, `Destroy` (idempotent). |
| `sandbox/sandboxtest/contract.go` | The one suite every backend must pass: stream capture, exit codes, workdir, timeout-kills-and-survives, output cap, binary file round-trip with parent creation, missing/oversize/directory reads, shared filesystem between `Exec` and the file primitives, idempotent provision, final-and-idempotent destroy, and networking isolation. |
...
- **Timeouts are enforced inside the container.** Docker has no API to kill a running exec, so the wrapper script forks a watchdog that marks a file and then `kill -9`s. `TimedOut` therefore requires both the watchdog's exit code (124) *and* a deadline that actually elapsed — a command exiting 124 on its own is never misread. Timeouts round up to whole seconds (`sleep` takes no portable fractional argument). The watchdog's stdio goes to `/dev/null`: holding the exec's pipes kept the daemon's stream open for ~2.7s after every command exited.
...
- **Container hardening is not done.** No capability drops, no read-only rootfs, no non-root user. `HostConfig.Init` is set (the watchdog leaves short-lived orphans). The plan's shared-responsibility hardening is its own item.

**Test coverage:** the contract suite runs against a real Docker daemon (a missing daemon is a hard failure, not a skip — as with `pgtest`), using `debian:stable-slim`: the official `bash` image does *not* have `/bin/bash` (its bash is in `/usr/local/bin`), which is exactly the assumption the image contract pins. A scripted fake daemon over `httptest` covers what a real one will not reproduce on demand: image-missing → pull → retry, a lost create race adopting the winner's container, a pull failure delivered inside a 200 stream, garbled and non-JSON replies, start/exec/inspect failures, an exec the daemon never stops calling running, context cancellation mid-poll, symlink and oversize and truncated archives, and the unix-socket transport itself. Every guard was mutation-tested (disable it → exactly its own test fails); one mutation exposed a real bug, since fixed: a non-container 404 from `WriteFile` was reported as `ErrNotFound`, sending the caller after a missing sandbox when the path was merely wrong.

**Coverage gate:** the CI denominator is now logic packages only — `internal/pgtest` and `internal/sandbox/sandboxtest` are excluded, for the same reason `cmd/` always was. They are packages solely because a test in another package must import them, and their uncovered statements are the assertion branches that fire only when a suite fails. Over the logic packages the number is **91.46%** against the unchanged ≥ 90% bar.
...
1. **Slice 6, the rest:** the `tool_exec` executor + built-in `agent_toolset_20260401` expansion (the queue kind, its enqueue seam, and now the sandbox all exist). Today the brain emits only `agent.custom_tool_use` (client-executed); `agent.tool_use` intents begin with the toolset expansion. The toolset's six sandbox-executed tools and their input schemas are pinned by the SDK checkout (`betaagent.go`): `bash{command?,restart?,timeout_ms?}`, `edit{file_path,old_string,new_string,replace_all?}`, `glob{pattern,path?}`, `grep{pattern,path?}`, `read{file_path,view_range?}`, `write{file_path,content}`. `web_fetch`/`web_search` are named in the tool-config enum but carry no input schema there and execute executor-side per the plan — defer them.
   ...
   **Also open:** nothing destroys a sandbox yet. Session termination (and reaping orphans by the `dev.opensdlc.managed-agent-platform.session-id` label) belongs with the executor.

...
the log is written in the order its events are processed, as the reference lists it. Two
PRs; #793 stays open until the second, and #539 closes with the first.
...
- [x] PR-A — a wake's running pair before the `user.message` or `user.define_outcome` it
      ...
      deliberate
- [ ] PR-B — a message posted mid-turn replays after the reply it never saw; its list and
      stream position stay where it was received

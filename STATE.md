...
- [ ] **Slice 1 — the `/v1/files` registry.** Migration `0008_files.sql`; `api/files.go` + `filesupload.go` + `filesmetrics.go`: upload (multipart, 500 MB cap, filename validation, `downloadable:false`), list (classic `Page` envelope + `after_id`/`before_id`/`limit`/`scope_id`), get-metadata, download (400 gate + streaming path), delete; routes + 405 fallbacks; end-to-end integration test over real Postgres + blob store; structured logging + metrics on every link.

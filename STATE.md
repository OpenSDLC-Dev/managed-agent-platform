...
| `wire.go` | Body parsing with omitted/null/value distinction (reference updates are patches), **strict unknown-field rejection** (typos error instead of silently vanishing, matching the reference's extra-inputs behavior), tools/skills/mcp_servers union validation (raw bodies preserved so configs round-trip byte-for-byte), UTC-normalized timestamps (`Z`, never a local offset). |

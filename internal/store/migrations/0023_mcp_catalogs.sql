...
-- mcp_exec: MCP discovery and execution run in the platform executor's own
-- process for cloud AND self_hosted sessions alike, so like web_exec the kind
-- list is the only schema change Claim needs — and Poll never serves it. The
-- CHECK is dropped and re-added under its auto-generated name, the
-- 0015_web_exec_kind pattern.
...
-- pass, read by the brain at request assembly.
--

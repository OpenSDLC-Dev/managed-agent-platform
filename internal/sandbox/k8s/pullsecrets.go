...
// The references land on the pod, not per-container, so the one knob covers the
// sandbox container and a gated session's net-setup and gate images alike.
//
// Parsed here rather than passed through so a name the API server would refuse
...
func parseImagePullSecrets(s string) ([]corev1.LocalObjectReference, error) {
	...
	for _, entry := range strings.Split(s, ",") {
		...
		// A Secret name is a DNS-1123 subdomain, and the server checks it. The
		// two habits this catches are uppercase and underscores — both legal in
		// the Docker config the credential came from, neither in a Secret name.

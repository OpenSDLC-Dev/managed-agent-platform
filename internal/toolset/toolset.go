...
// tool that succeeded silently. It is the reference runner's placeholder (its
// wording: "The Sessions API rejects empty text blocks; a tool that succeeds
// silently must still produce a postable result", since anthropic-sdk-go
// v1.63.1 — betasessiontoolrunner.go toToolResultContent), and one constant
// because the executor and the BYOC worker must post the same block for the
// same empty read.
...
// The check is lexical, like resolve: a symlink can lead a path out of its
// root — planted by `bash` where the agent has it, carried in by a repository
// clone where it does not — and what protects the store behind a read-only
// mount is the sync's pull-only mode rather than this check, which is here to
// be the clear answer the file tools owe a model that tries.

...
// TestIntegrationRealEndpoint drives one real model turn against an
// OpenAI-compatible endpoint configured in the environment (MODEL_PROTOCOL=openai
// / MODEL_BASE_URL / MODEL_API_KEY / MODEL_ID, optionally loaded from the repo-root
// .env). It skips — never fails — when no such endpoint is configured, so CI
// without credentials is unaffected. Credential values are never logged.
func TestIntegrationRealEndpoint(t *testing.T) {
	...
	if os.Getenv("MODEL_PROTOCOL") != "openai" || baseURL == "" || apiKey == "" || model == "" {
		t.Skip("no openai-protocol model endpoint configured (MODEL_* env)")
	}

	p, err := openai.New(provider.Config{
		Protocol: "openai", Model: model, BaseURL: baseURL, APIKey: apiKey,
	})
	if err != nil {
		t.Fatalf("New: %v", err)
	}

...
// The API contract tests run against a real Postgres started in Docker, the
// same pattern as internal/store. A missing Docker daemon is a hard failure,
// not a skip: skipped contract tests would silently hollow out the coverage
// gate.
...
func testMain(m *testing.M) int {
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "api tests require Docker for Postgres: %v\n", err)
		return 1
	}
...
func freshDB(t *testing.T) string {
	...
	name := fmt.Sprintf("api_test_%d", dbCounter.Add(1))

...
func TestFileUploadExpiresInSeconds(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"one below the minimum", "3599", false},
		{"one above the maximum", "7776001", false},
		{"zero", "0", false},
		{"negative", "-3600", false},
		{"empty", "", false},
		{"not a number", "an hour", false},
		{"fractional", "3600.0", false},
		{"padded", " 3600", false},
		{"scientific", "36e2", false},
		{"absurdly long", strings.Repeat("9", 40), false},
	} {
...
func TestExpiredFileLifecycle(t *testing.T) {
	...
	if err := s.blobs.Put(context.Background(), "files/"+genID,
	...
	if got := pageIDs(listData(t, body)); len(got) != 1 || got[0] != genID {

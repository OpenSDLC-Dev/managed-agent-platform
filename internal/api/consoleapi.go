...
//
// Auth needs no dispatcher change: every worker, gate and dual-auth predicate
// keys on a /v1/ prefix, so an /api/ path falls to dispatchAuth's management
// lane. The console's BFF holds that x-api-key server-side; it never reaches a
...
// deployment issuing more than a hundred keys to one environment can page.
const consoleKeyLimit = maxLimit

// environmentKeyNameMax bounds the operator's label. The dialect's own bound is
// unobserved, so this is a local choice: long enough for a hostname or a
// "staging-eu-west-1 runner" phrase, short enough that a listing stays readable
// and a name cannot be used as bulk storage.
...
// consoleEnvironment resolves the {organization_id}/{environment_id} pair every
// console-API environment route addresses, returning the environment's id and
// kind. An unrecognized organization, a malformed environment id and an absent
// environment all answer with the same 404 shape the wire uses, so the namespace
// is no better an enumeration oracle than /v1 is.
func (s *server) consoleEnvironment(r *http.Request) (id, kind string, archived bool, err error) {
	if org := r.PathValue("org"); org != reservedOrganization {
		return "", "", false, errNotFound("organization %s not found", org)
	}
	id = r.PathValue("id")
	...
	err = s.pool.QueryRow(r.Context(),
		`SELECT kind, archived_at FROM environments WHERE id = $1`, id).Scan(&kind, &archivedAt)
	if errors.Is(err, pgx.ErrNoRows) {
		return "", "", false, errNotFound("environment %s not found", id)
	}
	if err != nil {
		return "", "", false, err
	}
	return id, kind, archivedAt != nil, nil
}
...
func (s *server) createEnvironmentKey(r *http.Request) (any, error) {
	envID, kind, archived, err := s.consoleEnvironment(r)
	if err != nil {
		return nil, err
	}
	...
	if name == "" || len(name) > environmentKeyNameMax {
		return nil, errInvalid("name must be 1-%d characters", environmentKeyNameMax)
	}
	key, err := IssueEnvironmentKey(r.Context(), s.pool, envID, name)
	if err != nil {
		return nil, err
	}
...
// listEnvironmentKeys renders an environment's live keys. Kind and archive state
// are not consulted: an operator must be able to see, and revoke, keys already
// issued to an environment whatever has happened to it since.
func (s *server) listEnvironmentKeys(r *http.Request) (any, error) {
	envID, _, _, err := s.consoleEnvironment(r)
	if err != nil {
		return nil, err
	}

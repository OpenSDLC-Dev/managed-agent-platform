...
- **Leave no process or container of yours running.** Rungs 2 and 3 have you starting things — scratch copies, containers, load generators — and each one is yours to reap before you report. Confirm the reap with `ps` / `docker ps`; the exit status of the `kill` you just ran proves nothing. `LOADPIDS=$(jobs -p); kill $LOADPIDS` in particular collects nothing at all — the command substitution runs in a subshell with no jobs of its own — so capture each pid with `$!` as you spawn it. (That exact line orphaned twelve busy loops from a rung-2 stress probe on 2026-07-26; they burned twelve cores for two hours after the run reported PASS.) Anything you leave running deliberately — a cluster you did not create, a container that predates you — belongs in your report.

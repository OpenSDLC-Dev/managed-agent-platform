...
- **Re-derive the success criteria.** Read STATE.md and CLAUDE.md (and, for claims scoped to a plan, the relevant plan under `docs/plan/` — STATE.md's "Active plan" section names the current one). If the criteria you were given are weaker than what those documents require, verify against the documents and say so.

...
| `BAO_STATIC_KEY` / `BAO_PLATFORM_TOKEN` | Static-unseal key (base64, 32 bytes) and platform token for the bundled OpenBao (committed dev defaults; rotate via `.env` beyond a laptop). The `openbao-init` one-shot initializes the instance on first boot — root token and recovery keys land on the `baoinit` volume, a dev-grade convenience — and mints/renews the transit-scoped platform token the controlplane's and executor's `SECRETS_*`/`BAO_*` wiring follows automatically. |
